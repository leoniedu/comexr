...
Automatic downloading can be tricky, due to timeout, (lack of) valid security certificates on the Brazilian government websites, along other issues. 

Everytime one calls comexstat_download it resets the memoisation of the package (which is used to speed up results) and checks the local cache against the validation file in the website. It will download the data if this fails.
...
saldo_deflated_r <- saldo_deflated%>%
  ...
  filter(!is.na(vl_fob_current_usd))%>%
  mutate(
    vl_fob_current_usd_bi_deflated=
      slider::slide_index_dbl(.x=vl_fob_current_usd, 
                              .before = months(nperiods),
                              .complete = TRUE,
                              .f = function(z) sum(z, na.rm=TRUE), .i = co_ano_mes)/1e9,
    vl_fob_current_brl_bi_deflated=
      slider::slide_index_dbl(.x=vl_fob_current_brl, 
                              .before = months(nperiods),
                              .complete = TRUE,
                              .f = function(z) sum(z, na.rm=TRUE), .i = co_ano_mes)/1e9
    )%>%
  mutate(vl_fob_current_usd_bi_deflated_i=vl_fob_current_usd_bi_deflated/vl_fob_current_usd_bi_deflated[co_ano_mes=="2022-01-01"])

ggplot(aes(x=co_ano_mes, y=vl_fob_current_usd_bi_deflated, color=fluxo), 
...
```{r}
ggplot(aes(x=co_ano_mes, y=saldo_current_usd_bi_deflated), 
       data=
         saldo_deflated_r%>%
         group_by(co_ano_mes)%>%
         arrange(desc(fluxo))%>%
         summarise(saldo_current_usd_bi_deflated=vl_fob_current_usd_bi_deflated[2]-vl_fob_current_usd_bi_deflated[1])%>%
         na.omit()
                   )+
  #scale_color_manual(values=c("blue", "red")) +
  geom_line() +
...
```{r}
ggplot(aes(x=co_ano_mes, y=saldo_current_brl_bi_deflated), 
       data=
         saldo_deflated_r%>%
         group_by(co_ano_mes)%>%
         arrange(desc(fluxo))%>%
         summarise(saldo_current_brl_bi_deflated=vl_fob_current_brl_bi_deflated[2]-vl_fob_current_brl_bi_deflated[1])%>%
         na.omit()
                   )+
  #scale_color_manual(values=c("blue", "red")) +
  geom_line() +
